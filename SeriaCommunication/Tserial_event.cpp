		...
		char ByteSize, bool modem_events)
{
	...
	else
	{
		...
		thread_handle = (HANDLE) _beginthreadex(NULL, 0,
				(PBEGINTHREADEX_THREADFUNC) Tserial_event_thread_start, this, 0,
				&threadid);
		/*if (thread_handle==-1)
		 thread_handle=0;   */
	}
	...
	while (!done)
	{
		...
		else
		{
			...
			case SIG_READER:
				...
				//   );
				//
				if (GetOverlappedResult(serial_handle, &ovReader, &result_nbr,
						FALSE))
				{
#ifdef DEBUG_EVENTS
			...
			case SIG_WRITER:
				...
				if (GetOverlappedResult(serial_handle, &ovWriter, &result_nbr,
						FALSE))
				{
					// Write operation completed successfully
